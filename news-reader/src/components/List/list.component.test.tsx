...
describe('ListComponent', () => {
  ...
  xit('renders header with correct content', async () => {
    // Mock story details data
    const storyDetailsData: StoryDetail[] = [];
  ...
  xit('renders story container when storyDetails is not empty', async () => {
    // Mock story details data
  ...
  it('displays loading state when fetching stories', async () => {

    // Mock story details data
    const storyDetailsData = [
      { id: 1, title: 'Test Story 1', time: Date.now() / 1000, by: 'Test User 1', url: 'http://example.com/story1' },
      { id: 2, title: 'Test Story 2', time: Date.now() / 1000, by: 'Test User 2', url: 'http://example.com/story2' }
    ];

    const fetchStoryIds = jest.fn().mockResolvedValue([1, 2]);
    console.log(' storyDetailsData.find((story) => story.id === id)) ', () => async (id: number) => storyDetailsData.find((story) => story.id === id));
    const fetchStoryDetails = jest.fn().mockImplementation(() => ({ id: 1, title: 'Test Story 1', time: Date.now() / 1000, by: 'Test User 1', url: 'http://example.com/story1' }));
    ...
    expect(screen.getByTestId('story-container')).toContainElement(screen.getByTestId('loading-container'));
  });
});
